...
<div class="flex flex-col lg:flex-row gap-6">
    ...
    <main class="flex-1 min-w-0 max-w-2xl">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-6 animate-slide-up">Edit Kategori</h1>
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-xl shadow-sm border border-gray-100 p-6 card-glow-hover animate-slide-up-2">
            <form method="POST" action="{{ route('admin.categories.update', $category) }}">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" rows="3" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 bg-gray-50/50 focus:bg-white transition-all duration-300 @error('description') border-red-400 bg-red-50/50 @enderror">{{ old('description', $category->description) }}</textarea>
