...
<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="md:flex">
            <div class="md:w-1/2 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center p-8">
                @if($product->image)
                    <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-full h-full object-cover rounded-xl max-h-96">
                @else
                    <div class="text-center">
                        <svg class="w-32 h-32 text-gray-400 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
            ...
            <div class="p-8 md:w-1/2 flex flex-col">
                ...
                @if($product->stock > 0)
                    <form action="{{ route('cart.store') }}" method="POST" class="mt-6 pt-6 border-t border-gray-100">
                        ...
                        <button type="submit" class="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3.5 rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all font-semibold shadow-sm hover:shadow-md flex items-center justify-center gap-2">
        ...
        <div class="mt-12">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                @foreach($relatedProducts as $related)
                    <div class="group bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
                        <a href="{{ route('products.show', $related) }}">
                            <div class="h-36 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                                @if($related->image)
                                    <img src="{{ $related->image_url }}" alt="" class="w-full h-full object-cover">
                                @else
