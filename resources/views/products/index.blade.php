...
@section('content')
<div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 rounded-2xl p-8 sm:p-12 mb-10 text-white card-glow">
    <div class="relative z-10 animate-slide-up">
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            <span class="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full text-sm font-medium hover:bg-white/30 transition">
    ...
    <div class="absolute -bottom-10 -left-10 w-48 h-48 bg-white/10 rounded-full blur-3xl animate-float-delayed"></div>
</div>

<div class="flex flex-col lg:flex-row gap-6 mb-8">
    <div class="lg:w-60 shrink-0 animate-slide-up">
        <div class="bg-white/80 backdrop-blur-xl rounded-xl shadow-sm border border-gray-100 p-5 card-glow-hover">
            ...
            <div class="space-y-1">
                <a href="{{ route('products.index') }}" class="flex items-center justify-between px-3 py-2 rounded-lg text-sm {{ !request('category') ? 'bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-50 transition' }}">
                    <span>Semua</span>
                ...
                @foreach($categories as $category)
                    <a href="{{ route('products.index', ['category' => $category->id, 'search' => request('search')]) }}"
                        class="flex items-center justify-between px-3 py-2 rounded-lg text-sm {{ request('category') == $category->id ? 'bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-50 transition' }}">
    ...
    <div class="flex-1 min-w-0">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 animate-slide-up-2">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative flex-1 sm:flex-none">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari produk..."
                            class="w-full sm:w-64 border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 bg-gray-50/50 focus:bg-white transition-all duration-300">
                    ...
                    <button type="submit" class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-2.5 rounded-xl hover:from-indigo-700 hover:to-purple-700 transition text-sm font-medium shadow-sm hover:shadow-md">Cari</button>
                </div>
            </form>
        </div>

        @if($products->isEmpty())
            <div class="bg-white/80 backdrop-blur-xl rounded-xl shadow-sm border border-gray-100 p-12 text-center animate-slide-up">
                ...
                <a href="{{ route('products.index') }}" class="mt-4 inline-block text-indigo-600 hover:text-indigo-700 font-medium hover:underline">Reset filter</a>
            </div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                @foreach($products as $index => $product)
                    <div class="group bg-white/80 backdrop-blur-xl rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col card-glow-hover" style="animation: slide-up 0.6s ease-out {{ $index * 0.05 }}s both;">
                        <a href="{{ route('products.show', $product) }}" class="relative overflow-hidden">
                            ...
                            @if($product->stock < 1)
                                <div class="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center">
                                    <span class="bg-red-500 text-white px-4 py-1.5 rounded-full text-sm font-bold shadow-lg">Stok Habis</span>
                                </div>
                            @endif
                        </a>
                        <div class="p-4 flex flex-col flex-1">
                            ...
                            <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                                <span class="text-lg font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                                <span class="text-xs {{ $product->stock > 0 ? 'text-gray-400' : 'text-red-400' }}">
                            ...
                            <form action="{{ route('cart.store') }}" method="POST" class="mt-3">
                                ...
                                <button type="submit"
                                    class="w-full py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 {{ $product->stock > 0 ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-700 hover:to-purple-700 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 btn-shimmer' : 'bg-gray-200 text-gray-400 cursor-not-allowed' }}"
